...
오늘은 `safe lab와 sample handling`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `악성코드 분석 전 안전 절차 체크리스트`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 악성 샘플 실행이나 네트워크 연결 실험을 하지 않는다.

1. 오늘 주제인 `safe lab와 sample handling`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `safe lab`, `sample handling`, `VM isolation`, `snapshot` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-14-day-02/index.md`에 `악성코드 분석 전 안전 절차 체크리스트` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 safe lab와 sample handling에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
| VM isolation | `VM isolation`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| snapshot | `snapshot`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
...
| hashing | `hashing`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| evidence log | `evidence log`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
...
1. `safe lab와 sample handling`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `악성코드 분석 전 안전 절차 체크리스트`을 작성한다. 형태는 safe handling, behavior evidence, IOC/TTP가 있는 malware note이다.
...
1. 오늘 주제 `safe lab와 sample handling`를 한 문장으로 설명하면 무엇인가?
2. `safe lab`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `sample handling`와 `VM isolation`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
