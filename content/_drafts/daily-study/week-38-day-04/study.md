...
오늘은 `exploit chain와 mitigation bypass`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `exploit chain과 mitigation matrix`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 서비스 공격이나 공개 exploit 전개를 하지 않고 교육용 target에서만 실습한다.

1. 오늘 주제인 `exploit chain와 mitigation bypass`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `ROP`, `ret2libc`, `ASLR`, `NX` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-38-day-04/index.md`에 `exploit chain과 mitigation matrix` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 exploit chain와 mitigation bypass에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| exploit chain | 하나의 bug primitive가 leak, write, control-flow, sandbox escape 중 어떤 단계와 연결되는가? | chain diagram에 각 단계의 input, output, precondition을 적는다. | 단일 crash를 실제 공격 가능성과 분리해 평가한다. |
| mitigation bypass | ASLR/NX/canary/PIE/RELRO/seccomp 중 오늘 chain이 어떤 완화책을 상대하는가? | `checksec`, crash log, leak output, syscall restriction을 표로 정리한다. | exploit reliability와 report severity를 판단하는 기준이다. |
| info leak | 주소, heap/libc/kernel pointer, token 등 어떤 정보가 다음 단계의 전제인지 설명할 수 있는가? | leak source, leaked value, use site를 연결한다. | ASLR/KASLR 우회와 chain 안정성의 핵심이다. |
| ROP / ret2libc | code reuse가 shellcode 실행과 어떻게 다른지 설명할 수 있는가? | gadget/register state와 libc symbol 주소 계산을 기록한다. | NX 환경에서 control-flow primitive를 평가한다. |
| ASLR / PIE | base address randomization을 우회하려면 어떤 leak 또는 brute-force 조건이 필요한가? | 실행마다 base address 변화를 비교한다. | exploit 재현성의 가장 흔한 실패 원인이다. |
| canary / stack pivot | stack 보호와 pivot이 payload layout을 어떻게 바꾸는지 설명할 수 있는가? | crash 위치, canary overwrite 여부, pivot register를 기록한다. | chain 설계와 실패 triage에 필요하다. |
| seccomp / sandbox | syscall allowlist가 payload 목표를 어떻게 제한하는지 설명할 수 있는가? | seccomp profile 또는 denied syscall log를 확인한다. | exploit 후 post-exploitation 가능성을 과장하지 않게 한다. |
...
1. `exploit chain와 mitigation bypass`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `exploit chain과 mitigation matrix`을 작성한다. 형태는 target, trigger, root cause, primitive가 있는 vulnerability note이다.
...
1. 오늘 주제 `exploit chain와 mitigation bypass`를 한 문장으로 설명하면 무엇인가?
2. `ROP`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `ret2libc`와 `ASLR`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
