...
오늘은 `ret2win 기초`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `ret2win payload 구조와 성공 조건`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 서비스 공격이나 공개 exploit 전개를 하지 않고 교육용 target에서만 실습한다.

1. 오늘 주제인 `ret2win 기초`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `ret2win`, `return address overwrite`, `calling convention`, `stack alignment` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-19-day-04/index.md`에 `ret2win payload 구조와 성공 조건` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 ret2win 기초에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| ret2win | `ret2win`가 어떤 exploit primitive나 mitigation과 연결되는가? | 교육용 target의 crash, register, stack, mitigation, payload layout을 확인한다. | 취약점 원인 분석, exploit reliability, patch/detection 판단에 필요하다. |
...
| win function | `win function`가 어떤 exploit primitive나 mitigation과 연결되는가? | 교육용 target의 crash, register, stack, mitigation, payload layout을 확인한다. | 취약점 원인 분석, exploit reliability, patch/detection 판단에 필요하다. |
| payload layout | `payload layout`가 어떤 exploit primitive나 mitigation과 연결되는가? | 교육용 target의 crash, register, stack, mitigation, payload layout을 확인한다. | 취약점 원인 분석, exploit reliability, patch/detection 판단에 필요하다. |
...
1. `ret2win 기초`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `ret2win payload 구조와 성공 조건`을 작성한다. 형태는 target, trigger, root cause, primitive가 있는 vulnerability note이다.
...
1. 오늘 주제 `ret2win 기초`를 한 문장으로 설명하면 무엇인가?
2. `ret2win`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `return address overwrite`와 `calling convention`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
