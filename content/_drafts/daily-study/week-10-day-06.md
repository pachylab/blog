---
title: "Week 10 Day 06: network forensics와 exfiltration pattern"
draft: true
---

# Week 10 Day 06: network forensics와 exfiltration pattern

## 오늘의 목표

- network forensics, traffic timeline, exfiltration pattern를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- large upload, DNS tunneling intro, session reconstruction가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
network forensics, traffic timeline, exfiltration pattern, large upload, DNS tunneling intro, session reconstruction

## 반드시 정리할 개념

- network forensics: network forensics의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
- traffic timeline: traffic timeline의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
...
2. traffic timeline를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
3. exfiltration pattern를 단일 IOC가 아니라 시간 패턴, TLS metadata, byte volume으로 판단하려면 어떤 증거가 필요한가?
4. large upload를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
5. DNS tunneling intro를 PCAP에서 필터링하려면 어떤 필드를 보고 정상/비정상 판단 근거는 무엇인가?
6. session reconstruction를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
7. 오늘 산출물인 "PCAP 기반 traffic timeline"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
