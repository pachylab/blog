...
오늘은 `Windows security model: token, SID, ACL, UAC`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `Process Explorer와 AccessChk로 본 token/integrity/ACL 표`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: Windows 내부 구조 전체를 외우려 하지 말고 오늘 구조체나 artifact 하나만 본다.

1. 오늘 주제인 `Windows security model: token, SID, ACL, UAC`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `Windows process`, `Windows thread`, `job object`, `access token` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-24-day-02/index.md`에 `Process Explorer와 AccessChk로 본 token/integrity/ACL 표` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 Windows security model: token, SID, ACL, UAC에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| access token | process/thread가 어떤 사용자와 privilege로 실행되는지 담는 권한 컨텍스트다. | Process Explorer, `whoami /all`, token view로 SID, privilege, integrity를 확인한다. | malware 권한 상승, lateral movement, DFIR account attribution의 기준이 된다. |
| SID | Windows principal을 식별하는 고유 ID다. 이름 표시보다 SID가 더 안정적인 증거다. | `whoami /user`, ACL entry, event log subject SID를 비교한다. | 계정명 변경이나 local/domain 계정 혼동을 줄인다. |
| ACL / DACL | object에 누가 어떤 access mask를 가질지 정하는 접근 제어 목록이다. | AccessChk, `icacls`, object permission view로 ACE를 확인한다. | misconfiguration, privilege escalation, persistence path 분석에 필요하다. |
| privilege | token에 부여된 특수 권한이다. 권한 이름이 있어도 enabled 상태인지 따로 봐야 한다. | `whoami /priv`, Process Explorer token tab으로 enabled/disabled를 구분한다. | SeDebugPrivilege 같은 권한 남용 가능성을 판단한다. |
| integrity level | UAC와 Mandatory Integrity Control에서 쓰는 신뢰 수준이다. | Process Explorer integrity column과 token detail을 확인한다. | low/medium/high integrity 사이의 write boundary를 설명한다. |
| UAC split token | 관리자 계정도 기본적으로 filtered token으로 실행되고 elevation 때 full token을 받는다. | 관리자 shell과 일반 shell의 token, privilege, integrity를 비교한다. | "관리자 계정인데 왜 실패했는가"를 설명하는 핵심이다. |
| access check | subject token, object security descriptor, requested access를 비교해 허용 여부를 결정한다. | 같은 파일/registry key에 대해 user, elevated admin, SYSTEM 관찰을 비교한다. | exploit 후 권한 효과와 defensive policy 효과를 재현 가능하게 설명한다. |

## 실습

오늘 실습은 크게 벌리지 않는다. 아래 항목 중 오늘 산출물에 필요한 최소 범위만 수행하고, 실패하면 실패 로그를 그대로 남긴다.

- Process Explorer 또는 AccessChk/icacls 출력 하나를 골라 token, SID, ACL, integrity level이 어디에 나타나는지 표시한다.
- 같은 객체에 대해 일반 사용자와 elevated admin에서 접근 결과가 어떻게 달라지는지 기록한다.
...
1. `Windows security model: token, SID, ACL, UAC`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `Process Explorer와 AccessChk로 본 token/integrity/ACL 표`을 작성한다. 형태는 subject/object/boundary/telemetry가 있는 privilege-boundary note이다.
...
1. 오늘 주제 `Windows security model: token, SID, ACL, UAC`를 한 문장으로 설명하면 무엇인가?
2. `Windows process`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `Windows thread`와 `job object`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
