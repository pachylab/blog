---
title: "Week 07 Day 07: 주간 복습과 자동 분석 연결"
draft: true
---

# Week 07 Day 07: 주간 복습과 자동 분석 연결

## 오늘의 목표

- compiler, linker, symbol를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- relocation, CFG, bytecode가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
compiler, linker, symbol, relocation, CFG, bytecode, Python scripting
...
2. linker 단계에서 생성되거나 소비되는 artifact는 무엇이고 reversing에서는 어디서 확인하는가?
3. symbol 단계에서 생성되거나 소비되는 artifact는 무엇이고 reversing에서는 어디서 확인하는가?
4. relocation 단계에서 생성되거나 소비되는 artifact는 무엇이고 reversing에서는 어디서 확인하는가?
5. CFG 결과를 도구가 만든 값 그대로 믿지 않기 위해 어떤 교차 검증을 했는가?
6. bytecode를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
7. 오늘 산출물인 "Week 07 빌드/분석 자동화 체크리스트"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
