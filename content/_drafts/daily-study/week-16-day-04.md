...
- process hollowing, APC injection, suspended process를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- section mapping, entry point, memory permission가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
- process hollowing: process/thread/task를 주소 공간 공유 여부와 scheduler 단위로 구분한다. context switch 때 저장/복원되는 상태를 정리한다.
- APC injection: 기법을 실행 시점, 권한, API 단계, host artifact, event log로 분해한다.
- suspended process: process/thread/task를 주소 공간 공유 여부와 scheduler 단위로 구분한다. context switch 때 저장/복원되는 상태를 정리한다.
- section mapping: section mapping의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
...
1. process hollowing를 관찰하기 위해 어떤 OS 도구를 썼고 출력에서 PID/TID/state/resource를 어떻게 읽었는가?
2. APC injection 기법이 남기는 host artifact와 event log를 최소 2개 이상 연결할 수 있는가?
3. suspended process를 관찰하기 위해 어떤 OS 도구를 썼고 출력에서 PID/TID/state/resource를 어떻게 읽었는가?
4. section mapping를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
