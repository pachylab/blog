---
title: "Week 35 Day 05: probability, statistics, entropy"
draft: true
---

# Week 35 Day 05: probability, statistics, entropy

## 오늘의 목표

오늘은 `probability, statistics, entropy`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `entropy와 FP를 이용한 탐지 판단 예시`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 증명 전체를 완주하려 하지 말고, 오늘 실습에 필요한 정의와 예제만 다룬다.

1. 오늘 주제인 `probability, statistics, entropy`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `probability`, `statistics`, `entropy`, `information theory` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-35-day-05/index.md`에 `entropy와 FP를 이용한 탐지 판단 예시` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 probability, statistics, entropy에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
probability, statistics, entropy, information theory, base rate, false positive, anomaly detection
...
|---|---|---|---|
| probability | `probability`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| statistics | `statistics`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| entropy | `entropy`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| information theory | `information theory`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| base rate | `base rate`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| false positive | `false positive`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
| anomaly detection | `anomaly detection`를 손으로 계산하거나 작은 예제로 확인할 수 있는가? | 표, truth table, 작은 Python 코드, 그래프 그림 중 하나로 확인한다. | crypto, symbolic execution, detection rule, graph/call graph 해석의 기초가 된다. |
...
1. `probability, statistics, entropy`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `entropy와 FP를 이용한 탐지 판단 예시`을 작성한다. 형태는 problem, method, complexity, validation이 있는 algorithm note이다.
...
1. 오늘 주제 `probability, statistics, entropy`를 한 문장으로 설명하면 무엇인가?
2. `probability`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `statistics`와 `entropy`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
...
7. 오늘 산출물인 "entropy와 FP를 이용한 탐지 판단 예시"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
