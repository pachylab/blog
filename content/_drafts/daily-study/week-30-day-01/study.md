...
오늘은 `Android/iOS app sandbox`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `mobile app sandbox artifact 표`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 클라우드 계정 실험을 새로 벌이지 말고 공개 예시나 무해한 로그 조각으로 분석한다.

1. 오늘 주제인 `Android/iOS app sandbox`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `mobile forensics`, `Android app sandbox`, `iOS app sandbox`, `app data directory` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-30-day-01/index.md`에 `mobile app sandbox artifact 표` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 Android/iOS app sandbox에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| mobile forensics | `mobile forensics`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| Android app sandbox | `Android app sandbox`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| iOS app sandbox | `iOS app sandbox`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| app data directory | `app data directory`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| permissions | `permissions`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| keychain intro | `keychain intro`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| backup artifact | `backup artifact`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
...
1. `Android/iOS app sandbox`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `mobile app sandbox artifact 표`을 작성한다. 형태는 artifact, timestamp, confidence가 있는 evidence note이다.
...
1. 오늘 주제 `Android/iOS app sandbox`를 한 문장으로 설명하면 무엇인가?
2. `mobile forensics`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `Android app sandbox`와 `iOS app sandbox`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
