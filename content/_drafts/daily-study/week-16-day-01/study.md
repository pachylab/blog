...
오늘은 `Run key, service, scheduled task`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `지속성 기법별 artifact 표`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 허가되지 않은 실제 악성 샘플 실행이나 외부 네트워크 연결을 하지 않는다.

1. 오늘 주제인 `Run key, service, scheduled task`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `persistence`, `Run key`, `Windows service`, `scheduled task` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-16-day-01/index.md`에 `지속성 기법별 artifact 표` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 Run key, service, scheduled task에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| persistence | `persistence`가 malware behavior나 artifact에서 어떻게 관찰되는가? | 안전한 lab note, toy sample, report excerpt, log artifact 중 하나로 확인한다. | IOC/TTP, detection rule, incident scoping, report evidence로 이어진다. |
| Run key | `Run key`가 malware behavior나 artifact에서 어떻게 관찰되는가? | 안전한 lab note, toy sample, report excerpt, log artifact 중 하나로 확인한다. | IOC/TTP, detection rule, incident scoping, report evidence로 이어진다. |
| Windows service | `Windows service`가 malware behavior나 artifact에서 어떻게 관찰되는가? | 안전한 lab note, toy sample, report excerpt, log artifact 중 하나로 확인한다. | IOC/TTP, detection rule, incident scoping, report evidence로 이어진다. |
...
| startup folder | `startup folder`가 malware behavior나 artifact에서 어떻게 관찰되는가? | 안전한 lab note, toy sample, report excerpt, log artifact 중 하나로 확인한다. | IOC/TTP, detection rule, incident scoping, report evidence로 이어진다. |
...
1. `Run key, service, scheduled task`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `지속성 기법별 artifact 표`을 작성한다. 형태는 safe handling, behavior evidence, IOC/TTP가 있는 malware note이다.
...
1. 오늘 주제 `Run key, service, scheduled task`를 한 문장으로 설명하면 무엇인가?
2. `persistence`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `Run key`와 `Windows service`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
