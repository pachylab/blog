...
오늘은 `project scope와 safe lab 재확인`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `프로젝트 분석 계획서`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 악성 샘플 실행이나 네트워크 연결 실험을 하지 않는다.

1. 오늘 주제인 `project scope와 safe lab 재확인`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `project scope`, `safe lab`, `sample handling`, `VM snapshot` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-37-day-01/index.md`에 `프로젝트 분석 계획서` 초안을 채운다.
6. 성공 기준: 분석 대상, 격리 방식, hash, snapshot, 공개 가능 여부를 체크리스트로 남긴다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 project scope와 safe lab 재확인에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
### Primary 후보
- Learning Malware Analysis 또는 Practical Malware Analysis: lab safety, sample handling 관련 절
- Malware Unicorn: malware analysis methodology와 lab setup 관련 절
- 디지털 포렌식과 사고 대응 2/e: evidence handling, chain of custody 관련 절

### Reference
- REMnux docs: isolated analysis VM, FakeDNS, INetSim 개요
- VirtualBox/VMware docs: snapshot, host-only network, shared folder risk
- hashing 도구 문서: shasum, sha256sum, Get-FileHash
...
오늘의 primary 추천: malware lab safety 또는 forensic evidence 자료 중 안전 원칙 1개와 기록 양식 1개만 고른다.
...
|---|---|---|---|
| project scope | `project scope`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| safe lab | `safe lab`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| sample handling | `sample handling`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| VM snapshot | `VM snapshot`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| network isolation | `network isolation`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| analysis question | `analysis question`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |
| evidence plan | `evidence plan`가 분석 환경의 어떤 위험을 줄이는가? | VM 설정, snapshot, network mode, hash, evidence log를 확인한다. | malware/DFIR 실습에서 오염, 유출, 증거 훼손을 방지한다. |

## 실습

오늘 실습은 크게 벌리지 않는다. 아래 항목 중 오늘 산출물에 필요한 최소 범위만 수행하고, 실패하면 실패 로그를 그대로 남긴다.

- 분석 VM 또는 가상 실습 폴더의 경계, 네트워크, 공유 폴더 상태를 기록한다.
- 샘플 대신 무해한 테스트 파일로 hash, 경로, 보관 위치, 공개 가능 여부를 기록한다.
- 실패하거나 불확실한 안전 설정은 Unknown으로 남기고 다음 확인 방법을 적는다.
...
1. `project scope와 safe lab 재확인`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
2. 그 대상을 처음 확인할 때 어떤 명령어, 도구, 문서 항목을 먼저 봐야 할까?
3. 출력에 상세 정보, 줄 번호, hex view, filter, field, version 정보를 추가하려면 어떤 옵션이나 기록 항목이 필요할까?
4. 오늘 환경에서 host와 guest 사이에 데이터가 이동할 수 있는 경로는 무엇인가?
5. 오늘 결과를 블로그 글로 바꿀 때 공개하면 안 되는 경로, 샘플, 토큰, exploit detail, 개인정보는 무엇인가?

## 오늘 남길 산출물

- 강의 산출물: `프로젝트 분석 계획서`을 작성한다. 형태는 safe handling, behavior evidence, IOC/TTP가 있는 malware note이다.
...
1. 오늘 주제 `project scope와 safe lab 재확인`를 한 문장으로 설명하면 무엇인가?
2. `project scope`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `safe lab`와 `sample handling`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
4. sample handling 기록에서 빠지면 나중에 분석 신뢰도를 떨어뜨리는 항목은 무엇인가?
