...
오늘은 `registry, service, WMI`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `registry/service/WMI artifact map`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: Windows 내부 구조 전체를 외우려 하지 말고 오늘 구조체나 artifact 하나만 본다.

1. 오늘 주제인 `registry, service, WMI`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `registry`, `service`, `Service Control Manager`, `WMI` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-24-day-05/index.md`에 `registry/service/WMI artifact map` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 registry, service, WMI에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| registry | `registry`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| service | `service`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| Service Control Manager | `Service Control Manager`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| WMI | `WMI`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| event log | `event log`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| autorun | `autorun`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
| persistence | `persistence`가 Windows 내부 구조나 telemetry에서 어떻게 관찰되는가? | Sysinternals, WinDbg, event log, registry artifact 중 하나로 확인한다. | malware persistence, DFIR timeline, EDR telemetry, privilege boundary 해석에 필요하다. |
...
1. `registry, service, WMI`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `registry/service/WMI artifact map`을 작성한다. 형태는 subject/object/boundary/telemetry가 있는 privilege-boundary note이다.
...
1. 오늘 주제 `registry, service, WMI`를 한 문장으로 설명하면 무엇인가?
2. `registry`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `service`와 `Service Control Manager`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
