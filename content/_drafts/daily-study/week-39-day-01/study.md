...
오늘은 `CVE/N-day reading protocol`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `논문/CVE/N-day 읽기 템플릿 작성본`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 신고나 공개를 완료 조건으로 삼지 않고, 학습용 advisory-grade 기록을 만든다.

1. 오늘 주제인 `CVE/N-day reading protocol`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `research paper`, `CVE advisory`, `N-day`, `vulnerability class` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-39-day-01/index.md`에 `논문/CVE/N-day 읽기 템플릿 작성본` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 CVE/N-day reading protocol에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| research paper | `research paper`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| CVE advisory | `CVE advisory`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| N-day | `N-day`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| vulnerability class | `vulnerability class`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| CWE | `CWE`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| CPE | `CPE`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| affected version | `affected version`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
| threat model | `threat model`가 보고서의 어떤 항목과 근거를 요구하는가? | 공식 기준, advisory, 체크리스트, evidence table을 확인한다. | 공개 가능한 주장과 비공개 근거를 분리하는 기준이 된다. |
...
1. `CVE/N-day reading protocol`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `논문/CVE/N-day 읽기 템플릿 작성본`을 작성한다. 형태는 target, trigger, root cause, primitive가 있는 vulnerability note이다.
...
1. 오늘 주제 `CVE/N-day reading protocol`를 한 문장으로 설명하면 무엇인가?
2. `research paper`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `CVE advisory`와 `N-day`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
