...
- safe lab, VM isolation, snapshot를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- host-only network, sample handling, evidence가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
- host-only network: host-only network의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
...
2. VM isolation를 지키지 않으면 host 오염, 증거 훼손, 네트워크 유출 중 어떤 위험이 생기는가?
3. snapshot를 지키지 않으면 host 오염, 증거 훼손, 네트워크 유출 중 어떤 위험이 생기는가?
4. host-only network 영역에서 내가 설명할 수 있는 개념, 직접 해본 실습, 남길 수 있는 산출물을 각각 하나씩 말할 수 있는가?
5. sample handling를 지키지 않으면 host 오염, 증거 훼손, 네트워크 유출 중 어떤 위험이 생기는가?
6. evidence를 기록할 때 hash, 시간, 도구 버전, 분석자, 원본/복제본 구분 중 무엇이 빠지면 신뢰성이 약해지는가?
7. 오늘 산출물인 "개인 분석 랩 안전 체크리스트"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
