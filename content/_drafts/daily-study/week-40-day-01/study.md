...
오늘은 `40주 개념 coverage audit`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `도메인별 강점/공백 audit 표`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 실제 신고나 공개를 완료 조건으로 삼지 않고, 학습용 advisory-grade 기록을 만든다.

1. 오늘 주제인 `40주 개념 coverage audit`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `coverage audit`, `computer architecture`, `operating system`, `network` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-40-day-01/index.md`에 `도메인별 강점/공백 audit 표` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 40주 개념 coverage audit에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
|---|---|---|---|
| coverage audit | 40주 동안 실제로 다룬 개념과 산출물이 무엇인가? | roadmap, daily-study, worksheet 산출물 제목을 도메인별로 세어 본다. | 강점/공백을 근거로 후속 12주 방향을 정한다. |
| computer architecture | bit/byte, CPU, register, ABI, memory layout 근거 산출물이 있는가? | Week 01-05, 34-35 산출물 링크와 부족한 항목을 표시한다. | pwn, RE, kernel 분석의 선행 기반을 점검한다. |
| operating system | process, VM, filesystem, Windows/Linux internals 산출물이 있는가? | Week 04-05, 08, 24-25, 26-30 산출물을 매핑한다. | malware/DFIR/system hacking에서 필요한 OS 기반 공백을 찾는다. |
| network | TCP/IP, DNS/HTTP/TLS, PCAP, IDS, C2 산출물이 있는가? | Week 09-10, 18, 29 산출물을 evidence로 연결한다. | C2 분석, network forensics, detection engineering 기반을 점검한다. |
| compiler | C, compiler/linker/loader, IR/CFG/SSA, runtime reversing 산출물이 있는가? | Week 06-07, 11, 36 산출물을 확인한다. | exploit root cause와 decompiler output 검증 능력을 점검한다. |
| cryptography | encoding, hash, HMAC, AES, RSA/ECC, KDF, randomness 산출물이 있는가? | Week 31-33, 35 산출물을 확인한다. | malware config, ransomware, TLS/PKI 분석의 기초 공백을 찾는다. |
| reversing | PE/ELF, debugger, xref, CFG, obfuscation, runtime 분석 산출물이 있는가? | Week 11-13, 17, 36-37 산출물을 확인한다. | 악성코드/취약점 분석 포트폴리오의 깊이를 점검한다. |
| DFIR | evidence, Windows artifact, memory, network, cloud/container/identity 산출물이 있는가? | Week 08, 26-30, 37 산출물을 확인한다. | incident timeline과 evidence confidence 공백을 찾는다. |

## 실습

오늘 실습은 크게 벌리지 않는다. 아래 항목 중 오늘 산출물에 필요한 최소 범위만 수행하고, 실패하면 실패 로그를 그대로 남긴다.

- 40주 roadmap과 daily-study 산출물 목록을 도메인별로 대조한다.
- 각 도메인마다 "충분한 산출물 1개"와 "공백 1개"를 적는다.
- 공백은 즉시 보충하려 하지 말고 후속 12주 후보 또는 유지 복습 후보로 분류한다.
...
date
git status --short
rg -n "Week 19|Week 24|Week 28|Week 38" content/_drafts/study-plan/security-study-roadmap/index.md
rg -n "kernel|Windows|malware|DFIR|fuzzing|CVE" content/_drafts/daily-study -g "study.md"
find content/_drafts/daily-study -name "study.md" | wc -l
```

### 스스로 답할 질문

1. `40주 개념 coverage audit`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `도메인별 강점/공백 audit 표`을 작성한다. 형태는 public claim, evidence, private boundary가 분리된 capstone artifact이다.
...
1. 오늘 주제 `40주 개념 coverage audit`를 한 문장으로 설명하면 무엇인가?
2. `coverage audit`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `computer architecture`와 `operating system`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
