---
title: "Week 31 Day 06: hash-based IOC와 malware triage"
draft: true
---

# Week 31 Day 06: hash-based IOC와 malware triage

## 오늘의 목표

- hash-based IOC, fuzzy hash intro, sample identity를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- collision risk, file metadata, YARA complement가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
hash-based IOC, fuzzy hash intro, sample identity, collision risk, file metadata, YARA complement

## 반드시 정리할 개념

- hash-based IOC: malware taxonomy를 기능, 전달 방식, persistence, C2 필요성으로 구분하고 IOC와 TTP를 분리한다.
- fuzzy hash intro: fuzzy hash intro의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
- sample identity: sample identity의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
- collision risk: collision risk의 정의, 관찰 가능한 artifact, 보안 분석에서 쓰이는 지점, 혼동하기 쉬운 개념을 2-3문장으로 정리한다.
- file metadata: content와 metadata를 분리하고 inode/MFT/journal/timestamp가 삭제, 이동, 실행 흔적을 어떻게 남기는지 정리한다.
- YARA complement: YARA는 file/content, Sigma는 log/event 중심이라는 차이를 정리하고 rule 목적, 조건, false positive 관리 방법을 적는다.
...
2. fuzzy hash intro를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
3. sample identity를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
4. collision risk를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
5. file metadata artifact가 증명할 수 있는 행위와 증명할 수 없는 행위는 무엇인가?
6. YARA complement를 보고서나 rule로 쓸 때 false positive를 줄이기 위한 조건은 무엇인가?
7. 오늘 산출물인 "hash IOC의 장단점 표"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
