---
title: "Week 05 Day 01: virtual address space와 physical memory"
draft: true
---

# Week 05 Day 01: virtual address space와 physical memory

## 오늘의 목표

오늘은 `virtual address space와 physical memory`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `virtual-to-physical translation 흐름도`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: exploit 완성까지 가지 말고, memory layout과 보호기법 관찰에 집중한다.

1. 오늘 주제인 `virtual address space와 physical memory`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `virtual address space`, `virtual address`, `physical memory`, `MMU` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-05-day-01/index.md`에 `virtual-to-physical translation 흐름도` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 virtual address space와 physical memory에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
virtual address space, virtual address, physical memory, MMU, address translation, page table
...
|---|---|---|---|
| virtual address space | `virtual address space`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
| virtual address | `virtual address`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
| physical memory | `physical memory`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
| MMU | `MMU`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
| address translation | `address translation`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
| page table | `page table`가 address space, permission, mitigation 중 무엇을 설명하는가? | proc maps, pmap, GDB, checksec, crash log 중 하나로 확인한다. | exploitability 판단과 memory forensic artifact 해석에 필요하다. |
...
1. `virtual address space와 physical memory`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `virtual-to-physical translation 흐름도`을 작성한다. 형태는 상태 변화와 관찰 증거가 연결된 system behavior note이다.
...
1. 오늘 주제 `virtual address space와 physical memory`를 한 문장으로 설명하면 무엇인가?
2. `virtual address space`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `virtual address`와 `physical memory`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
...
7. 오늘 산출물인 "virtual-to-physical translation 흐름도"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
