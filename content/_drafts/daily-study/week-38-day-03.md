...
- exploit primitive, out-of-bounds write, arbitrary read를 자료의 정의, 실습 관찰, 보안 분석 쓰임으로 나누어 설명한다.
- arbitrary write, control-flow hijack, information leak가 실제 artifact, primitive, IOC, evidence 중 어디에 연결되는지 예시를 남긴다.
...
exploit primitive, out-of-bounds write, arbitrary read, arbitrary write, control-flow hijack, information leak
...
3. arbitrary read를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
4. arbitrary write를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
5. control-flow hijack를 오늘 실습 결과와 연결해 설명하면 입력, 처리 과정, 출력, 한계는 무엇인가?
