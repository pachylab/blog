---
title: "Week 30 Day 04: Microsoft Entra와 sign-in/audit logs"
draft: true
---

# Week 30 Day 04: Microsoft Entra와 sign-in/audit logs

## 오늘의 목표

오늘은 `Microsoft Entra와 sign-in/audit logs`을 넓게 훑는 날이 아니라, 작은 입력이나 예제 하나로 관찰하고 기록하는 날이다. 오늘 끝나면 `identity incident timeline 초안`에 들어갈 최소 근거가 남아 있어야 한다.

오늘 하지 않을 것: 클라우드 계정 실험을 새로 벌이지 말고 공개 예시나 무해한 로그 조각으로 분석한다.

1. 오늘 주제인 `Microsoft Entra와 sign-in/audit logs`가 무엇을 다루는지 한 문장으로 적는다.
2. 핵심 키워드 `Entra`, `sign-in log`, `audit log`, `conditional access` 중 3개를 골라 "정의, 관찰 방법, 보안 분석에서의 쓰임" 형태로 정리한다.
3. Primary 자료는 1개만 고르고, 실제로 읽은 범위와 이해 안 된 문장 1개를 남긴다.
4. 실습을 실행한다. 도구가 없으면 설치부터 하지 말고, 필요한 입력/명령어/옵션/예상 출력을 먼저 설계한다.
5. 공부 후 결과물 worksheet `week-30-day-04/index.md`에 `identity incident timeline 초안` 초안을 채운다.
...
| 중단 후 복귀 | 새 자료를 열지 말고 Microsoft Entra와 sign-in/audit logs에서 남길 수 있는 가장 작은 근거 1개만 worksheet에 적는다. |
...
Entra, sign-in log, audit log, conditional access, MFA, service principal, OAuth consent
...
|---|---|---|---|
| Entra | `Entra`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| sign-in log | `sign-in log`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| audit log | `audit log`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| conditional access | `conditional access`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| MFA | `MFA`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| service principal | `service principal`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
| OAuth consent | `OAuth consent`가 mobile/cloud/container artifact에서 어떤 행위 단서를 제공하는가? | 공식 log field, sample event, SQLite/plist row, container log 중 하나를 확인한다. | incident scoping, identity correlation, cloud/mobile DFIR report에 필요하다. |
...
1. `Microsoft Entra와 sign-in/audit logs`을 확인하기 위한 가장 작은 입력 파일, 샘플, 코드, 로그, 보고서 조각은 무엇인가?
...
- 강의 산출물: `identity incident timeline 초안`을 작성한다. 형태는 artifact, timestamp, confidence가 있는 evidence note이다.
...
1. 오늘 주제 `Microsoft Entra와 sign-in/audit logs`를 한 문장으로 설명하면 무엇인가?
2. `Entra`는 어떤 입력, 출력, field, instruction, artifact에서 확인했는가?
3. `sign-in log`와 `audit log`를 혼동하면 어떤 분석 오류가 생길 수 있는가?
...
7. 오늘 산출물인 "identity incident timeline 초안"이 다음 분석 단계에서 재사용 가능하려면 어떤 정보가 더 필요한가?
